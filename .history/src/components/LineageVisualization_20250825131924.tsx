  ...
  const generateHierarchicalLayout = useCallback((
    ...
    const collectLineage = (nodeId: string, isUpstream: boolean) => {
      ...
      if (nodeId !== rootId) {
        if (isUpstream) {
          upstream.add(nodeId);
          nodeLevels.set(nodeId, -1); // All upstream nodes at level -1
        } else {
          downstream.add(nodeId);
          nodeLevels.set(nodeId, 1); // All downstream nodes at level 1
        }
      } else {
        nodeLevels.set(nodeId, 0); // Root node at level 0
      }
