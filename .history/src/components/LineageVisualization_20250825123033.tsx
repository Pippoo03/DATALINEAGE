...
export const LineageVisualization: React.FC<LineageVisualizationProps> = ({
  ...
  const generateLineage = useCallback(() => {
    ...
    const buildGraph = (nodeId: string, level: number, position: { x: number, y: number }) => {
      ...
      upstreamEdges.forEach((edge, index) => {
        if (!visitedNodes.has(edge.source)) {
          const xOffset = (index - (upstreamEdges.length - 1) / 2) * HORIZONTAL_SPACING;
          buildGraph(
            edge.source,
            level - 1,
            { 
      ...
      downstreamEdges.forEach((edge, index) => {
        if (!visitedNodes.has(edge.target)) {
          const xOffset = (index - (downstreamEdges.length - 1) / 2) * HORIZONTAL_SPACING;
          buildGraph(
            edge.target,
            level + 1,
            { 
    ...
    buildGraph(config.selectedComponentId, 0, { x: 0, y: 0 });
